...
class SHA384 final	// cppcheck-suppress noConstructor
{
...
public:
	...
	[[nodiscard]] inline std::string getHash()
	{
		...
		static constexpr unsigned DIGEST_SIZE = (384 / 8);
...
std::string sha384 (const char* input, std::size_t length)
{
	SHA384 hasher;
...
std::string sha384 (const std::string_view& input)
{
	return sha384 (input.data(), input.length());
}
