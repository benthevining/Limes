...
void SHA384::update (const unsigned char* data, std::size_t length)
{
...
std::string SHA384::getHash()
{
	...
	static constinit const unsigned DIGEST_SIZE = (384 / 8);
...
void SHA384::transform (const unsigned char* message, unsigned block_nb) noexcept
{
	std::uint64_t w[80];
	std::uint64_t wv[8];
