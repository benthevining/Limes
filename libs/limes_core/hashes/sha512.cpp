...
class SHA512 final	// cppcheck-suppress noConstructor
{
	static constexpr unsigned SHA384_512_BLOCK_SIZE = (1024 / 8);
...
public:
	...
	[[nodiscard]] inline std::string getHash()
	{
		...
		static constexpr unsigned DIGEST_SIZE = (512 / 8);
...
private:
	...
	inline void transform (const unsigned char* message, unsigned block_nb)
	{
		std::uint64_t w[80];
		std::uint64_t wv[8];
		...
		{
			const auto* const sub_block = message + (i << 7);
			...
			for (auto j = 0; j < 80; ++j)
			{
				const auto t1 = wv[7] + SHA512_F2 (wv[4]) + SHA2_CH (wv[4], wv[5], wv[6])
							  + sha512_k[j] + w[j];

				const auto t2 = SHA512_F1 (wv[0]) + SHA2_MAJ (wv[0], wv[1], wv[2]);
	...
	static inline void SHA2_PACK64 (const unsigned char* str, std::uint64_t& x) noexcept
	{
	...
	static inline void SHA2_UNPACK32 (std::uint64_t x, unsigned char* str) noexcept
	{
	...
	static inline void SHA2_UNPACK64 (std::uint64_t x, unsigned char* str) noexcept
	{
	...
	static inline std::uint64_t SHA512_F4 (std::uint64_t x) noexcept
	{
		return SHA2_ROTR (x, 19) ^ SHA2_ROTR (x, 61) ^ (x >> 6);
	}

	static inline std::uint64_t SHA512_F3 (std::uint64_t x) noexcept
	{
		return SHA2_ROTR (x, 1) ^ SHA2_ROTR (x, 8) ^ (x >> 7);
	}

	static inline std::uint64_t SHA512_F2 (std::uint64_t x) noexcept
	{
		return SHA2_ROTR (x, 14) ^ SHA2_ROTR (x, 18) ^ SHA2_ROTR (x, 41);
	}

	static inline std::uint64_t SHA512_F1 (std::uint64_t x) noexcept
	{
		return SHA2_ROTR (x, 28) ^ SHA2_ROTR (x, 34) ^ SHA2_ROTR (x, 39);
	}

	static inline std::uint64_t SHA2_CH (std::uint64_t x, std::uint64_t y, std::uint64_t z) noexcept
	{
		return (x & y) ^ (~x & z);
	}

	static inline std::uint64_t SHA2_MAJ (std::uint64_t x, std::uint64_t y, std::uint64_t z) noexcept
	{
		return (x & y) ^ (x & z) ^ (y & z);
	}

	static inline std::uint64_t SHA2_ROTR (std::uint64_t x, std::uint64_t n) noexcept
	{
		return (x >> n) | (x << ((sizeof (x) << 3) - n));
	}

	static constexpr std::uint64_t sha512_k[80] = { 0x428a2f98d728ae22ULL, 0x7137449123ef65cdULL,
...
std::string sha512 (const std::string_view& input)
{
