...
std::vector<Country> Language::getCountries() const
{
...
struct KnownLanguages final
{
	...
	[[nodiscard]] std::vector<Language> getLanguagesForCountryCode (const std::string& code) const
	{
		LIMES_ASSERT (code.size() == 2);
		...
		for (const auto& lang : languages)
			if (alg::contains (lang.countryCodes, code))
				langs.push_back (lang);	 // cppcheck-suppress useStlAlgorithm

		return langs;
	}

	[[nodiscard]] const std::vector<Language>& getAll() const
	{
		return languages;
	}
...
std::vector<Language> getLanguagesForCountry (const std::string_view& countryCode)
{
	return getLanguages().getLanguagesForCountryCode (std::string { countryCode });
}

const std::vector<Language>& getAllKnownLanguages()
{
